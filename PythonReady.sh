...
LoggedInUser=$(who | cut -d ' ' -f1) 
read -n1 -p "Would you like to open any programs?[Y/n] " yn
while [[ $yn == "Y" || $yn == "y" ]]; do
        echo -e "\n1. Firefox \n2. Pluma \n3. Quit"
        read -n1 -p "Type the number of the program: " num
        if [ $num == 1 ]; then
                su $LoggedInUser -c 'firefox https://1332970.netacad.com/courses/1035863' &
        elif [ $num == 2 ]; then
                su $LoggedInUser -c pluma &
        elif [ $num == 3 ]; then
                echo -e "\n"
                exit
        fi
